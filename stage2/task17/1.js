...
function getDateStr(dat) {
  var y = dat.getFullYear();  // 获取年数
  var m = dat.getMonth() + 1; // 获取月数
  m = m < 10 ? '0' + m : m;
  var d = dat.getDate();      // 获取天数
  d = d < 10 ? '0' + d : d;
  return y + '-' + m + '-' + d;
}
...
var apiChartWrap = document.getElementsByClassName('aqi-chart-wrap')[0]
...
function initGraTimeForm() {
  ...
  for(var i = 0, len = pageRadio.length; i < len; i++) {
...
function initAqiChartData() {
  ...
  // nowCityData 是确定一个城市的92天降水数组, key 是日期, nowCityData[key] 是降水量

  if(pageState.nowGraTime == 'day') { // 选择 '天'
    chartData = nowCityData
  }

  if(pageState.nowGraTime == 'week') {  // 选择 '星期'
    chartData =  {}
